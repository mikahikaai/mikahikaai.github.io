...
if (isset($_POST['button_create'])) {
  $insertsql = "INSERT INTO obat (id_obat, nama_obat, jenis_obat, harga_jual, minimal_stok, stok_obat, khasiat, efek_samping) values (NULL,?,?,?,?,?,?,?)";
  $stmt = $db->prepare($insertsql);
  $stmt->bindParam(1, $_POST['nama_obat']);
  $stmt->bindParam(2, $_POST['jenis_obat']);
  $stmt->bindParam(3, $_POST['harga_jual']);
  $stmt->bindParam(4, $_POST['minimal_stok']);
  $stmt->bindParam(5, $_POST['stok_obat']);
  $stmt->bindParam(6, $_POST['khasiat']);
  $stmt->bindParam(7, $_POST['efek_samping']);
  if ($stmt->execute()) {
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <br>
              <textarea style="text-transform: uppercase;" name="khasiat" rows="4"><?= isset($_POST['button_create']) ? $_POST['khasiat'] : '' ?></textarea>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="efek_samping">Efek Samping</label>
              <textarea style="text-transform: uppercase;" name="efek_samping" rows="4"><?= isset($_POST['button_create']) ? $_POST['efek_samping'] : '' ?></textarea>
            </div>
          </div>
        </div>
